    ...
    v-if="!this.plsWait && !this.isDelete && !loadError">
      ...
      style="margin-bottom: 16px;">
        ...
        class="fullWidth fRow jStart aStart wrap" v-bind:class="{err: emptyError}">
          <div v-for="item in fullPermissionList" v-bind:key="item.id" class="checkGroup">
...
.checkBoxHolder{
  ...
  margin: unset;
  border-right: solid 1px rgba($color: #000000, $alpha: 0.4);
}
.checkBoxHolder.level2{
...
.checkGroup .subs{
  ...
  z-index: 2000;
  display: none;
  transition: all cubic-bezier(0.5, 0, 0, 1) 0.4s;
  background-color: rgb(250, 250, 250);
  box-shadow: 0 16px 48px 0 rgba($color: #000000, $alpha: 0.15);
}

.checkGroup:hover .subs{
  display: flex;
}
