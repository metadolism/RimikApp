<template>
  <div class="fullWidth wrap fRow aStart jAround">
    ...
    v-if="!this.plsWait && this.isDelete && !this.loadError">
      ...
      icon="delete_sweep" @click.native="deleteCourse"/>
    ...
    deleteCourse() {
      axios({
        ...
        if (response.data.isSuccess) {
          this.$store.commit('closeCard');
          this.changeMsg();
          this.makeToast('success', 'دوره مورد نظر با موفقیت حذف شد', 2000);
        }
      }, (error) => {
        // console.log(error);
        this.makeToast('error', this.errorHandler(error), 2000);
    ...
    init() {
      switch (this.mode) {
        ...
        case 'delete': {
          this.plsWait = false;
          this.isDelete = true;
          this.ct = `آیا از حذف  «${this.$store.state.focusCourseName}» مطمئن هستید؟`;
