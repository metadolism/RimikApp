  ...
  <div class="fullWidth wrap fRow aStart jAround">
    ...
    v-if="!this.plsWait && !this.isDelete && !loadError">
      ...
      <div class="fCol jStart aStart" style="min-width: 330px; margin-right: 8px;">
        <div class="imgError"
        v-if="lessonPhotoError">شما باید یک عکس برای پایه‌ی تحصیلی اضافه کنید</div>
        <get-photo
          w='100%'
          v-on:photoupload="getUploaded"
          photoFor='lesson'
          :initImg='this.computedInitImage'
          uploadMsg='آپلود عکس برای پایه‌ی تحصیلی الزامیست'
    ...
    v-if="!this.plsWait && this.isDelete && !this.loadError">
      ...
      icon="delete_sweep" @click.native="deleteTeacher"/>
    ...
    deleteTeacher() {
      axios({
        ...
        if (response.data.isSuccess) {
          this.$store.commit('closeCard');
          this.changeMsg();
          this.makeToast('success', 'دبیر مورد نظر با موفقیت حذف شد', 2000);
        }
      }, (error) => {
        // console.log(error);
        this.makeToast('error', this.errorHandler(error), 2000);
    ...
    init() {
      switch (this.mode) {
        ...
        case 'delete': {
          this.plsWait = false;
          this.isDelete = true;
          this.ct = `آیا از حذف  «${this.$store.state.focusTeacherName}» مطمئن هستید؟`;
